 ...
 * the main thread - it will be using the "regular" stack and PC. You may
 * assume that this function is called before any other thread library
 ...
 * -1 otherwise. If a thread terminates itself or the main thread is
 ...
 * quantum of the thread which has made the call to uthread_sleep isn’t
 * counted. It is considered an error if the main thread (tid == 0) calls this
 * function.
 *
 * @return On success, return 0. On failure, return -1.
 */
 ...
 * additional quantum that the thread starts should increase this value by 1
 * (so if the thread with ID tid is in RUNNING state when this function is
 * called, include also the current quantum). If no thread with ID tid exists
